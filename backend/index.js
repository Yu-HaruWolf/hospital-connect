...
//緯度経度を用いて救急車から目的地の病院までの距離を算出し病院のリストを距離を注目し昇順にソートする関数
//（引数1:救急車が選択した科に当てはまる病院のドキュメント)
...
service.getDistanceMatrix({
    ...
    //hospitalListを距離に注目して昇順ソート
    hospitalList.sort((a, b) => a[1] - b[1]);
...
calculateDistance(hospitals);//緯度経度から距離を算出する関数を実行
...
const depJapaneseList = [];
const depEnglishList = [];
const querySnapshot = await getDocs(departmentRef);
querySnapshot.forEach((doc) => {
    depJapaneseList.push(doc.data().ja);
    depEnglishList.push(doc.data().en);
});
//console.log(depJapaneseList);
//console.log(depEnglishList);
};
getDepartment();